        ...
        (let ((body (+ (current-column) 2)))
          ...
          (let ((head (buffer-substring-no-properties
            ...
            (if (or (eolp) (looking-at "#")
                    (member head '("begin" "defmacro" "lambda" "loop"))
...
(defvar vrs-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-c C-s") #'vrs-browse-services)
...
(autoload 'vrs-browse-services "vrs-choose" nil t)
