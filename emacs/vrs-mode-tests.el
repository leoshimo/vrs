...
(ert-deftest vrs-evaluation-with-test-runtime ()
  ...
  (let ((vrs-vrsctl-command (getenv "VRS_TEST_VRSCTL"))
    ...
    (with-temp-buffer
      ...
      (let ((vrs-result-width 90)) (vrs-insert-evaluated-code))
      (should (equal (buffer-string) "(focus_window '(:id 7))"))
      (erase-buffer)
      (insert "'(unquote @name)")
      (vrs-insert-evaluated-code)
...
(ert-deftest vrs-retention-and-code-insertion-with-test-runtime ()
  (skip-unless (getenv "VRS_TEST_VRSCTL"))
  (let ((vrs-vrsctl-command (getenv "VRS_TEST_VRSCTL")))
    ...
    (with-temp-buffer
      ...
      (insert "(self)")
      (should-error (vrs-eval-last-sexp t) :type 'user-error)
      (should (equal (buffer-string) "(self)")))
    (with-temp-buffer
      (vrs-mode)
      (insert "(def inserted_runs 0)\n'(set inserted_runs (+ inserted_runs 1))")
      (goto-char (point-min))
      (push-mark (point-max) t t)
      (let ((transient-mark-mode t)) (vrs-insert-evaluated-code))
      (should (equal (buffer-string) "(set inserted_runs (+ inserted_runs 1))"))
      (erase-buffer)
