...
Evaluate Lyric in a running VRS, keep values in your source, and use those
values to build or execute the next call. This guide covers the Emacs workflow;
the examples use a small notebook service you define below.
