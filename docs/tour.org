...
expression from the REPL can be included directly in a user interface
as an action. User interactions, in turn, can be recorded directly into
...
VRS is a live, personal software ecosystem, grown one small script at a time.
The whole environment is available from every client, on every connected device.

* Working through the tour

With [[file:../README.md][VRS running]], the examples work in the =vrsctl= REPL. For live
editing, use [[file:guide-emacs.org][the editor]]:
...
Each =:on_click= contains the same expression that could be typed at the
REPL, with its arguments already included. Add two actions: create a todo,
or create a todo with context from Safari.
...
Selecting an item executes its =:on_click= expression—the same code a
program would run. Use the runtime's built-in /pubsub/ to inspect those
...
A todo service starts as a small script. It can be extended to use the
browser, trigger automations, and power desktop and terminal user interfaces.
Its capabilities are available to other programs and clients, whether it
runs locally or on another device.
...
at the REPL and included directly in a user interface as an action.
