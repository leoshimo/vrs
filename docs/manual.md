...
`command_title` uses the first nonblank line of a function's docstring as
its label, falling back to the function name when there is no summary.
The full docstring remains available through `help` and `meta`.
...
Events are delivered to subscribers on connected nodes and are best-effort;
publishing does not wait for handlers to finish.
...
The source takes no arguments and returns a list of entities. Bindings import
sources exported by a service. To inspect a function or its sources:

```vrs
(meta focus_window)
(entity_sources :os/window) # => (get_windows)
```

The type annotation guides the picker; it does not reject other values when
you call the function directly.

Given an entity, `interactive_functions` returns bound function names whose first
`interactive` argument matches its leading tag. It runs neither those functions
nor entity sources. Emacs action selection and vrsjmp's Cmd-K menu use
this same query:

```vrs
(interactive_functions '(:os/window :id 7))
# => (focus_window) # Other bound window actions may also appear.
```

`entities` invokes registered sources for current values of a type:

```vrs
(entities :os/window)
```

The sources must be available in the calling environment. `interactive_functions`
finds operations from a value; `entities` supplies values for an
...
(register_entity_source :demo/item 'demo_items)
(register_entity_source :demo/place 'demo_places)
