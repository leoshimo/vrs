...
SOURCES = ('docs/index.md', 'docs/tour.md', 'docs/design.md',
           'docs/design-experience.md', 'docs/manual.md')
ASSETS = ('docs/site.css', 'docs/site.js',
          'docs/fonts/charter-regular.woff2', 'docs/fonts/charter-bold.woff2',
...
def rewrite_links(document, source, output):
    exports = {ROOT / name: Path(name).with_suffix('.html') for name in SOURCES}
    assets = set()
...
def book_layout(document, source, output):
    ...
    exports = {Path(name).stem: Path(name).with_suffix('.html') for name in SOURCES}
...
def build(destination):
    with tempfile.TemporaryDirectory(prefix='vrs-doc-export-') as temporary:
        ...
        copied_assets = set()
        for name in SOURCES:
            source, output = ROOT / name, Path(name).with_suffix('.html')
