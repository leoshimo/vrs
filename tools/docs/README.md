...
[`landing.html`](landing.html); its styles and behavior
live in `assets/landing.css` and `assets/landing.js`. The selected visual assets and
regeneration instructions are in [`tools/visuals`](../visuals/README.md).

The exporter lives in [`build.py`](build.py). It adds
the shared page layout, navigation, repository link, and code highlighting.
[`site.css`](assets/site.css) styles the pages; [`site.js`](assets/site.js) handles the contents
...
after an edit. A failed build leaves the last good preview in place and reports
...
Tests live in [`test_build.py`](test_build.py) and run
