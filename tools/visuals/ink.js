...
export function inkEffect(host) {
  ...
  let item, frame = 0, time = 0, last = 0, paused = true, visible = true;
  let canvas;
  ...
  function paint() {
    ...
    const center = .5 + .5 * Math.sin(time * .45);
    for (let y = 0; y < height; y++) {
      const scan = Math.exp(-Math.pow((y / height - center) / .085, 2));
      for (let x = 0; x < width; x++) {
        const n = y * width + x;
        const wave = Math.pow((1 + Math.sin(x / width * Math.PI * 2 - time * .95)) / 2, 4);
        const offset = wave * .65 * Math.sin(time * .7) + scan * 1.65 * Math.sin(time * 1.2);
  ...
  return {ready, setPaused(value) { paused = value; wake(); }};
}
